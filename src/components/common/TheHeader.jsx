...
const TheHeader = (props) => {
  ...
  function setCountryCode(code) {
    ...
    window.location.replace(
      `${origin}/${localStorage.code}/${localStorage.locale || "en"}/${pathname}`
    );
  }
  return (
    <div
    ...
    >
      ...
      <ul className={`header_list ${props.color}`}>
        <NavLink to={`/${code || 'NG'}/${language}/`}>
          <div className="kobo360 nav-item pointer">
            ...
            <div className="dropdown-content">
              <NavLink className="" to={`/${code || 'NG'}/${language}/about-kobo360`}>
              ...
              <NavLink className="" to={`/${code || 'NG'}/${language}/services`}>
                <div className="span dropdown">
                  <i className="fa fa-caret-right"></i>
                  <span>
                    {" "}
        ...
        <NavLink
          className="header-underline capitalize"
          to={`/${code || 'NG'}/${language}/kobocare`}
        >
          <FormattedMessage id="app.kobocare" defaultMessage="GoTruckCare" />
        </NavLink>
      ...
      <div className={`kobo360 nav-item pointer set-flex ${props.color} ${toggle ? 'none' : ''} `}>
        {code || 'NG'}
        {countries.length > 0 && countries.find(country => country.countryCode.toLowerCase() === ((code && code.toLowerCase()) || 'ng')) ? <img loading="lazy" src={countries.find(country => country.countryCode.toLowerCase() === ((code && code.toLowerCase()) || 'ng') ).flag} alt="flag" /> : ''}
        &nbsp;
        <i className="fa fa-caret-down"></i>
        <div className="dropdown-content country-dropdown" style={{ top: '2em' }}>
          {
            countries.length > 0 && countries.map(({ country, countryCode }, i) => <div
              onClick={() => setCountryCode(countryCode)} className={`country ${countryCode === code ? 'active' : ''} `} key={i}>{country}</div>)
          }
        </div>
        ...
        <div className="white pointer font-5 nav-list center">
          ...
          <div className='mobile-sel'>
            ...
            <i className="fa fa-caret-down"></i>
              <div className="dropdown-content country-dropdown" style={{ top: '2em' }}>
                {
                  countries.length > 0 && countries.map(({ country, countryCode }, i) => <div onClick={() => setCountryCode(countryCode)} className={`country ${countryCode === code ? 'active' : ''} `} key={i}>{country}</div>)
                }
              </div>
